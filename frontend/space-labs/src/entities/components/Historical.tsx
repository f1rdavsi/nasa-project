  ...
  <div className="space-y-3 animate-pulse">
    {[...Array(5)].map((_, i) => (
      <div key={i} className="flex gap-4">
        {[...Array(9)].map((_, j) => (
          <div key={j} className="h-8 bg-blue-900/30 rounded flex-1 shadow-[0_0_10px_rgba(30,58,138,0.3)]"></div>
        ))}
      </div>
    ))}
  </div>
...
export default function Historical() {
  ...
  const downloadCSV = () => {
    ...
    const csvContent = [
      headers.join(','),
      ...pollutionData.map(d => [d.date, d.aqi, d.pm25, d.pm10, d.co, d.no2, d.o3, d.so2].join(',')),
    ].join('\n')
    ...
    <div className="container mx-auto px-4 py-12 space-y-8">
      ...
      <Card
        ...
        <h2 className="text-2xl  font-orbitron font-bold text-cyan-400 mb-6">
          Air pollution data - {COUNTRIES.find(c => c.code === selectedCountry)?.name}
        </h2>
        {isLoading ? (
          <TableSkeleton />
        ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b-2 border-blue-800">
                    <th className="px-4 py-3 text-left text-cyan-400 font-semibold">Date</th>
                    <th className="px-4 py-3 text-left text-cyan-400 font-semibold">AQI</th>
                    <th className="px-4 py-3 text-left text-cyan-400 font-semibold">Status</th>
                    <th className="px-4 py-3 text-left text-cyan-400 font-semibold">PM2.5</th>
                    <th className="px-4 py-3 text-left text-cyan-400 font-semibold">PM10</th>
                    <th className="px-4 py-3 text-left text-cyan-400 font-semibold">CO</th>
                    <th className="px-4 py-3 text-left text-cyan-400 font-semibold">NO2</th>
                    <th className="px-4 py-3 text-left text-cyan-400 font-semibold">O3</th>
                    <th className="px-4 py-3 text-left text-cyan-400 font-semibold">SO2</th>
                  </tr>
                </thead>
                <tbody ref={tableBodyRef}>
                  {currentItems.map((row, idx) => {
                    ...
                    return (
                      <tr key={idx} className="border-b border-blue-900/50 hover:bg-blue-950/40">
                        <td className="px-4 py-3 text-cyan-100">{row.date}</td>
                        <td className={`px-4 py-3 font-bold text-lg ${getAQIColor(row.aqi)}`}>{row.aqi}</td>
              ...
              {Array.from({ length: totalPages }, (_, i) => (
                <Button
                  ...
                  className={`px-3 py-1 rounded ${
                    currentPage === i + 1 ? 'bg-cyan-500 text-white' : 'bg-slate-800 text-cyan-100'
                  }`}>
                  {i + 1}
                </Button>
              ))}
      ...
      <div ref={chartsRef} className="space-y-8">
        {isLoading ? (
          ...
          <>
            <Card className="p-6">
              <h3 className="text-xl font-semibold font-orbitron text-cyan-300 mb-4">Air quality index (AQI)</h3>
              <ResponsiveContainer width="100%" height={350}>
                <LineChart data={pollutionData}>
                  ...
                  <Legend />
                  <Line type="monotone" dataKey="aqi" stroke="#06b6d4" strokeWidth={2} />
                </LineChart>
              </ResponsiveContainer>
            </Card>

            <Card className="p-6">
              <h3 className="text-xl font-orbitron font-semibold text-cyan-300 mb-4">PM2.5</h3>
              <ResponsiveContainer width="100%" height={350}>
                <AreaChart data={pollutionData}>
                  ...
                  <YAxis stroke="#0ea5e9" />
                  <Tooltip content={CustomTooltip} />
                  <Area type="monotone" dataKey="pm25" stroke="#facc15" fill="rgba(250,204,21,0.3)" />
                </AreaChart>
              </ResponsiveContainer>
            </Card>

            <Card className="p-6">
              <h3 className="text-xl font-orbitron font-semibold text-cyan-300 mb-4">PM10</h3>
              <ResponsiveContainer width="100%" height={350}>
            ...
            <Card className="p-6">
              <h3 className="text-xl font-orbitron font-semibold text-cyan-300 mb-4">NO2</h3>
              <ResponsiveContainer width="100%" height={350}>
                <ComposedChart data={pollutionData}>
                  ...
                  <Line type="monotone" dataKey="no2" stroke="#059669" strokeWidth={2} />
                </ComposedChart>
              </ResponsiveContainer>
            </Card>

            <Card className="p-6">
              <h3 className="text-xl font-orbitron font-semibold text-cyan-300 mb-4">O3</h3>
              <ResponsiveContainer width="100%" height={350}>
                <AreaChart data={pollutionData}>
                  <defs>
                  ...
                  <YAxis stroke="#0ea5e9" />
                  <Tooltip content={CustomTooltip} />
                  <Area type="monotone" dataKey="o3" stroke="#3b82f6" fill="url(#o3Gradient)" />
                </AreaChart>
              </ResponsiveContainer>
            </Card>

            <Card className="p-6">
              <h3 className="text-xl font-orbitron font-semibold text-cyan-300 mb-4">SO2</h3>
              <ResponsiveContainer width="100%" height={350}>
                <LineChart data={pollutionData}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#1e3a8a" opacity={0.2} />
                  <XAxis dataKey="date" stroke="#0ea5e9" />
            ...
            <Card className="p-6">
              <h3 className="text-xl font-orbitron font-semibold text-cyan-300 mb-4">CO</h3>
              <ResponsiveContainer width="100%" height={350}>
                <LineChart data={pollutionData}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#1e3a8a" opacity={0.2} />
                  <XAxis dataKey="date" stroke="#0ea5e9" />
                  <YAxis stroke="#0ea5e9" />
                  <Tooltip content={<CustomTooltip />} />
                  <ReferenceLine y={1.0} stroke="green" strokeDasharray="3 3" label="Safe" />
                  <ReferenceLine y={2.5} stroke="orange" strokeDasharray="3 3" label="Moderate" />
                  <ReferenceLine y={5.0} stroke="red" strokeDasharray="3 3" label="High" />
                  <Line type="monotone" dataKey="co" stroke="#fbbf24" strokeWidth={2} />
                </LineChart>
              </ResponsiveContainer>
            </Card>
          </>
